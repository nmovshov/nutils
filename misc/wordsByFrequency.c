...
/* A single word that may be hung on a list of words. */
    ...
    int frequency; /* Frequency in word list.*/
    };
...
word->frequency = 1; /* newly created word just appeared for the first time*/
return word;
}
...
int wordCompareFrequency(const void *va, const void *vb)
/* Compare two words by frequency. */
{
const struct word *a = *((struct word **)va);    /* Convert from void to actual type */
const struct word *b = *((struct word **)vb);
...
void wordsByFrequency(char *fileName)
/* Open file, read through it keeping track of each unique word,
 * and then print out words by frequency of occurrence. */
{
/* Stream through input file building up a hash of unique words with frequency. */
FILE *f = mustOpen(fileName, "r");
struct hash *hash = hashNew();
struct word *wordList = NULL;
char wordBuffer[128];
...
/* Sort list and output results to standard output. */
wordList = sortWordList(wordList, wordCompareFrequency);
struct word *word;
for (word = wordList; word != NULL; word = word->next)
    puts(word->string);
}
...
if (argc != 2)
    {
    fprintf(stderr, 
      "wordsByFrequency - Print all words in a file sorted by frequency.\n"
      "usage:\n"
      "   wordsByFrequency textFile\n"
      );
    die();
    }
// wordsBySize(argv[1]);
wordsByFrequency(argv[1]);
return 0;
}
