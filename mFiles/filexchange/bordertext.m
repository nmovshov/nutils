...
%
% Last release:
% V 2.06  10-09-2013  Support syntax: bordertext( 'top', 'hello', 'Color', [1 0 0], 'FontSize', 20 );
%
...
% V 2.06  10-09-2013  Support matlab syntax: bordertext( 'top', 'hello', 'Color', [1 0 0], 'FontSize', 20 );
%                     text.Margin = eps; coloured background looks nicer.

%function h = bordertext( pos, txt, FormatString )
%function h = bordertext( pos, txt, 'Property1', Value1, 'Property2', Value2, ... )
function h = bordertext( varargin )
VERSION = 'V 2.06  10-09-2013';
...
%     imagesc(rand(25),[-1 1]); colormap(gray); axis auto tight square
    ...
    bordertext([50  2.0],'h = bordertext( ''Position'', ''Text'' [,''FormatString'' ])','[r][13][Hc]');
    bordertext([50  1.5],'h = bordertext( ''Position'', ''Text'' [,''Property1'', Value1, ... ])','[r][13][Hc]');
    bordertext([50 -2.0], VERSION,'[m][13][Hc]');
...
for fn = fieldnames( FormatStr )',
    set( h, char(fn), FormatStr.(char(fn)) );
end
...
if strcmpi( pos, 'userdefined' ),
    ...
    if isfield( UserData, 'BordertextHandles' ),
        if ~isfield( UserData.BordertextHandles, 'userdefined' ),
            ...
            UserData.BordertextHandles.userdefined_xspan    = abs( ax(2) - ax(1) );
            UserData.BordertextHandles.userdefined_yspan    = abs( ax(4) - ax(3) );
            UserData.BordertextHandles.userdefined_fsize    = FormatStr.FontSize;
        else
