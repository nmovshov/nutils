function [A,ero] = gjref(A)
%GJREF REF matrix by the book
%   GJREF(A) returns a matrix in row echelon form that is row equivalent to
%   the input matrix A. Row echelon forms are not unique; the matrix returned
%   by GJREF(A) is the row echelon form that results from applying the
...
%
%   [A,ero] = GJREF(A) additionally returns a string array detailing the
